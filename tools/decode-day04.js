#!/usr/bin/env node
/* =====================================================================
   Decode Day 04 submission codes into rows for the "Degrees JS EW" tab.

   Usage:
     node tools/decode-day04.js codes.txt          -> prints a table
     node tools/decode-day04.js codes.txt --csv    -> CSV for the sheet
     node tools/decode-day04.js D4.1Q.ZZZ… --csv   -> a single code
   ...
   Day 04 has its own salt, code prefix and check list, so this reads
   codec-day04.js + checks-day04.json. Day 01–03 codes are rejected.
   ===================================================================== */

const fs = require("fs");
const path = require("path");
const { decodeSubmission } = require("./codec-day04.js");
const roster = require("./roster.json");
const meta = require("./checks-day04.json");   // { total, required, optional, labels[] }
...
if (!target) {
  console.error("Usage: node tools/decode-day04.js <codes.txt | code> [--csv]");
  process.exit(1);
}
...
if (asCsv) {
  console.log("Row,Full Name,Email,Day " + DAY + ",Level,Bonus,Submitted");
  final.forEach(r => {
  ...
  console.log("");
  console.log("  Day 0" + DAY + " — decoded submissions");
